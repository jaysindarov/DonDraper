    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-base font-bold text-white">My Generations</h2>
                <Link :href="route('generations.create')"
                    class="flex items-center gap-2 bg-gradient-to-r from-sky-500 to-cyan-600 hover:from-sky-400 hover:to-cyan-500 text-white font-semibold px-4 py-2 rounded-xl transition-all shadow-lg shadow-sky-500/20 text-sm">
        ...
        <div class="py-7 px-6 max-w-7xl mx-auto">
            ...
            <div v-if="generations.data.length === 0" class="text-center py-32">
                ...
                <h3 class="text-xl font-bold text-white mb-2">No generations yet</h3>
                <p class="text-gray-400 text-sm mb-6">Create your first AI-generated image or video.</p>
                <Link :href="route('generations.create')"
                    class="inline-flex items-center gap-2 bg-gradient-to-r from-sky-500 to-cyan-600 hover:from-sky-400 hover:to-cyan-500 text-white font-bold px-6 py-3 rounded-xl transition-all shadow-lg shadow-sky-500/20 text-sm">
                    Get Started
                </Link>
            </div>

            <div v-else>
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 mb-8">
                    <Link v-for="gen in generations.data" :key="gen.id" :href="route('generations.show', gen.id)"
                        class="group relative bg-[#0A0E1A] border border-white/5 hover:border-sky-500/25 rounded-2xl overflow-hidden transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-sky-500/8">
                        <div class="aspect-square bg-gradient-to-br from-gray-800 to-gray-900 relative overflow-hidden">
                            ...
                            <div v-else class="w-full h-full flex flex-col items-center justify-center gap-2">
                                <svg v-if="gen.status !== 'failed'" class="w-7 h-7 text-sky-400/50 animate-spin" fill="none" viewBox="0 0 24 24">
                        ...
                        <div class="px-3 py-2.5 flex items-center justify-between">
                            ...
                            <span class="text-[10px] text-gray-600">{{ new Date(gen.created_at).toLocaleDateString() }}</span>
                        </div>
                    </Link>
                </div>
